...
const DataTableMain = ({
  ...
  useEffect(() => {
    ...
    // if (selectionTemplate || headerSelection) {
  ...
  return (
      <div
        className={`hcl-table-wrapper${
          type.includes('borderless') ? ' hcl-data-table-borderless' : ''
        }`}
        {...restProps}
      >
        <table
                    ...
                    >
                      ...
                      column.field !== 'expand' ? (
                        <div className="table-body-content-wrapper">
                          {column.renderHtml
                            ? column.renderHtml(row)
                            : column.field.split('.').length > 1 ? getValue(row , column.field): row[column.field]}
                        </div>
                      ) : null}
...
DataTableMain.propTypes = {
  ...
  overflowTemplate: PropTypes.func,
