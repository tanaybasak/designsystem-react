...
describe('<Slider> component', () => {
  ...
  it('simulate change event in Slider and is called Twice', () => {
    ...
    const wrapper = mount(
      <Slider onChange={mockCallBack} withInputBox={false} />
    );
    wrapper
      .find(`.${prefix}-slider-input`)
      .simulate('change', { currentTarget: { value: 55 } });
  ...
  it('simulate blur, onchange event in inputbox of Slider', () => {
    ...
    expect(wrapper.find(`.${prefix}-slider-text-input`).props().value).toEqual(
      5
    );
  });
  ...
  it('aceept only Number in inputbox of Slider', () => {
    ...
    expect(
      wrapper.find(`.${prefix}-slider-text-input`).props().value
    ).not.toBeNaN();
  });
});
