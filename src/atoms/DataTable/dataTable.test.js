...
test('ability to apply custom class name',()=>{
    const wrapper = mount(<DataTable id='table_1' className="aishwarya" tableConfig={[{field:'name'},{field:'empId'}]} tableData={[{name:'employeeA',empId:'46867'}]}></DataTable>);
    expect(wrapper.find('.aishwarya').length).toBe(1);
    wrapper.unmount();
})
