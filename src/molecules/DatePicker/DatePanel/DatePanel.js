...
const DatePanel = ({ currDateObj, dateSelected, selectDate, format }) => {
    ...
    const createDateNodelist = () => {
        ...
        const numOfDaysInPrevMonth = getDaysInMonth(currDateObj.month === 0 ? 12 : currDateObj.month, currDateObj.month === 0 ? currDateObj.year - 1 : currDateObj.year);
    ...
    const createDayHTML = (type, i) => {
        ...
        switch (type) {
            case 'previous':
                month = (`0${currDateObj.month === 0 ? 12 : currDateObj.month}`).slice(-2);
                year = currDateObj.month === 0 ? currDateObj.year - 1 : currDateObj.year;
                break;
            case 'current':
                ...
                break;
            case 'next':
                month = (`0${Number(currDateObj.month === 11 ? -1 : currDateObj.month) + 2}`).slice(-2);
                year = currDateObj.month === 11 ? currDateObj.year + 1 : currDateObj.year;
        }
